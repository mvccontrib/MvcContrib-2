...
{
	[TestFixture]
	public class DelegatePaginationTester
	{

		private IPagination<T> CreatePagination<T>(IEnumerable<string> strings, int pageNum, int pageSize, object additionalArgs)
		{
			return new DelegatePagination<T>()
		}

		[Test]
		public void Should_create_pagination_with_default_page_size() {
			var strings = new List<string> { "First", "Second", "Third" };
			var pagination = CreatePagination(strings); strings.AsPagination(1);
